...
if (urls.length === 0) {
  console.error(
    "Usage: deno run --allow-net --allow-read --allow-write .script/process-moonbit-docs.ts <URL> [URL...]",
  );
  Deno.exit(1);
}
...
function parseSections(text: string): { path: string; content: string }[] {
  ...
  while ((match = sectionRegex.exec(text)) !== null) {
    matches.push({ path: match[1], index: match.index + match[0].length });
  }
...
function splitByHeading(
  ...
  if (headings.length <= 1) return [{ filename: baseFilename, content }];
  ...
  for (let i = 0; i < headings.length; i++) {
    ...
    result.push({ filename: fname, content: content.slice(start, end).trim() });
  }
  return result;
}
...
for (const section of sections) {
  if (section.path === "language/index.md") continue;
  ...
  const subFiles = isFundamentals
    ...
    : [{ filename: baseFilename, content: section.content }];
